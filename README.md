...
本地体验版按用户偏好使用官方 `latest` 基础镜像；其中 SSH 隧道是单一功能小镜像 `ssh-tunnel-client:local`，`FROM alpine:latest` 后只安装 `openssh-client`。`scripts/start-local-tunnel-stack.sh` 默认每次执行都会 `--pull always --build --force-recreate`。
