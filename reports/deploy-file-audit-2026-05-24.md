...
| 文件 | 结果 | 说明 |
|---|---|---|
| `.env.local-tunnel` | 已对齐 | 本地真实环境文件；MySQL/Redis 走 `ssh-tunnel` sidecar；已修正文档头，避免误写“不要提交”。 |
| `.env.local-tunnel.example` | 已对齐 | 本地模板文件；保留 `CHANGE_ME`；适合复制后填写。 |
| `.env.server` | 已对齐 | 服务器真实环境文件；Docker 后端访问宿主机 MySQL/Redis 使用 `host.docker.internal`。 |
| `.env.server.example` | 已对齐 | 服务器模板文件；保留 `CHANGE_ME`。 |
| `.gitignore` | 已对齐 | 只忽略 `*.local`；真实 `.env.*` 在此私有部署仓库中按当前约定保留。 |
| `NEED-INFO.md` | 已对齐 | 已确认信息和服务器上线前待确认项仍有效。 |
| `README.md` | 已对齐 | 已写明 `master-jdk17`、Java17 镜像、BPM 两处启用点、sidecar 隧道和 Nginx 502 修复。 |
| `backend/Dockerfile` | 已对齐 | 运行镜像为 `eclipse-temurin:17-jre`，Jar 路径仍是 `ruoyi-vue-pro/yudao-server/target/yudao-server.jar`。 |
| `docker-compose.local-tunnel.yml` | 已对齐 | 启动 `ssh-tunnel`、`server`、`frontend`；传入 `spring.data.redis.*` 和 `spring.redis.*`；挂载 Nginx 配置。 |
| `docker-compose.server.yml` | 已对齐 | 服务器不走 SSH 隧道；后端容器访问宿主机服务；挂载 Nginx 配置。 |
...
| `frontend/Dockerfile` | 已对齐 | 前端构建使用 `node:22-alpine`，运行镜像 `nginx:1.27-alpine`。 |
...
| `reports/ui-audit-local-tunnel-2026-05-23T18-52-26-748Z.json` | 保留 | Java17 后端下的正式浏览器巡检报告，57/57 通过。 |
| `reports/ui-audit-2026-05-23T18-52-26-748Z-home.png` | 保留 | 上述正式巡检的首页截图证据。 |
| `reports/ui-audit-local-tunnel-2026-05-24.md` | 保留 | 502 根因和修复记录。 |
| `reports/ui-menu-audit-local-tunnel-2026-05-23.json` | 已删除 | 旧报告包含 “BPM 已禁用”、`host.docker.internal:13306` 等 Java17 切换前状态，继续保留会误导。 |

## 额外发现

部署清单要求 `yudao-ui-admin-vue3` 中 6 个 BPM 页面顶部的 “工作流手册” 提示保持注释。审计时发现当前前端源码仍有 6 处未注释，已作为跨仓一致性问题处理。
